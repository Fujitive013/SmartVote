...
const Home = () => {
  ...
  useEffect(() => {
    // Only start polling if user data is loaded
    if (user?.city_id && user?.baranggay_id) {
      // Define the fetch function for polling
      const loadElections = () => {
        fetchElectionsData(user.city_id, user.baranggay_id);
      };

      // Immediately fetch data once when component loads
      loadElections();

      // Set up polling every 3 seconds (3000 ms)
      const intervalId = setInterval(loadElections, 3000);

      // Cleanup on unmount or when dependencies change
      return () => clearInterval(intervalId);
    }
  }, [user]); // dependencies: user data
